...
function calculateRightInset(period?: string, width: number, data: Array<Point>): number {
  ...
  const startOfDayTime = moment(currentTime).startOf('day').valueOf();
  const endOfDayTime = moment(startOfDayTime).add(1, 'day').valueOf();
...
class Chart extends React.PureComponent<ChartProps, ChartState> {
    ...
    render() {
      ...
      const isGrowing = newData[0].y <= newData[newData.length - 1].y;
      ...
      return (
        ...
        >
          <Background
            ...
            height={height}
            gradientOptions={
              (isGrowing ? gradientOptions.backgroundGrow : gradientOptions.backgroundDecline)
